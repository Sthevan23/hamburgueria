...
/* ---- Categories ---- */
router.get("/categories", requirePermission("menu"), async (req, res) => {
...
router.post("/categories", requirePermission("menu"), async (req, res) => {
  const rid = restaurantScope(req);
...
router.put("/categories/:id", requirePermission("menu"), async (req, res) => {
  const rid = restaurantScope(req);
  ...
  const cat = await db.prepare("SELECT * FROM categories WHERE id = ? AND restaurant_id = ?").get(req.params.id, rid);
  if (!cat) return res.status(404).json({ error: "Categoria não encontrada." });
  res.json(cat);
});

router.delete("/categories/:id", requirePermission("menu"), async (req, res) => {
  const rid = restaurantScope(req);
  const count = await db.prepare("SELECT COUNT(*) as c FROM products WHERE category_id = ?").get(req.params.id);
  if (count.c > 0) return res.status(400).json({ error: "Categoria possui produtos vinculados." });
  await db.prepare("DELETE FROM categories WHERE id = ? AND restaurant_id = ?").run(req.params.id, rid);
  res.json({ ok: true });
});

router.put("/categories/reorder", requirePermission("menu"), async (req, res) => {
  const rid = restaurantScope(req);
  const { order } = req.body;
  const stmt = db.prepare("UPDATE categories SET sort_order = ? WHERE id = ? AND restaurant_id = ?");
  for (let index = 0; index < order.length; index++) {
    await stmt.run(index, order[index], rid);
  }
  res.json({ ok: true });
});

/* ---- Products ---- */
router.get("/products", requirePermission("menu"), async (req, res) => {
  const rid = restaurantScope(req);
...
router.post("/products", requirePermission("menu"), async (req, res) => {
  const rid = restaurantScope(req);
...
router.put("/products/:id", requirePermission("menu"), async (req, res) => {
  const rid = restaurantScope(req);
  ...
  await db.prepare(`UPDATE products SET ${updates.join(", ")} WHERE id = ? AND restaurant_id = ?`).run(...values);
  const product = await db.prepare("SELECT * FROM products WHERE id = ? AND restaurant_id = ?").get(req.params.id, rid);
  if (!product) return res.status(404).json({ error: "Produto não encontrado." });
  res.json(product);
});

router.post("/products/:id/duplicate", requirePermission("menu"), async (req, res) => {
  const rid = restaurantScope(req);
  const original = await db.prepare("SELECT * FROM products WHERE id = ? AND restaurant_id = ?").get(req.params.id, rid);
...
router.delete("/products/:id", requirePermission("menu"), async (req, res) => {
  const rid = restaurantScope(req);
  await db.prepare("DELETE FROM products WHERE id = ? AND restaurant_id = ?").run(req.params.id, rid);
  res.json({ ok: true });
});

router.put("/products/reorder", requirePermission("menu"), async (req, res) => {
  const rid = restaurantScope(req);
  const { order } = req.body;

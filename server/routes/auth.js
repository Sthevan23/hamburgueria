...
router.get("/me", (req, res) => {
  ...
  try {
    ...
    const user = db.prepare("SELECT id, name, email, role, restaurant_id FROM users WHERE id = ?").get(payload.id);
    if (!user) return res.status(401).json({ error: "Usuário inválido." });
    const restaurant = db.prepare("SELECT id, name, is_open FROM restaurants WHERE id = ?").get(user.restaurant_id);

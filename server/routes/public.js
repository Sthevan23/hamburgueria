...
router.get("/menu", (req, res) => {
  const restaurant = db.prepare("SELECT * FROM restaurants WHERE slug = ?").get("burger-falcone");
...
router.get("/status", (req, res) => {
  const restaurant = db.prepare("SELECT is_open, closed_message FROM restaurants WHERE slug = ?").get("burger-falcone");
  if (!restaurant) return res.status(404).json({ error: "Restaurante não encontrado." });
...
router.post("/orders", (req, res) => {
  ...
  for (const item of orderItems) {
    ...
    for (const addon of item.addons) {
      insertAddon.run(itemResult.lastInsertRowid, addon.name, addon.price || 0);
    }
  }

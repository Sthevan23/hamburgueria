...
router.get("/", requirePermission("stock"), (req, res) => {
  const rid = restaurantScope(req);
  const items = db.prepare("SELECT * FROM stock WHERE restaurant_id = ? ORDER BY name").all(rid)
    .map((item) => ({ ...item, status: stockStatus(item) }));
  res.json(items);
});

router.post("/", requirePermission("stock"), (req, res) => {
  const rid = restaurantScope(req);
  ...
  res.status(201).json(db.prepare("SELECT * FROM stock WHERE id = ?").get(result.lastInsertRowid));
});

router.post("/:id/movement", requirePermission("stock"), (req, res) => {
  const rid = restaurantScope(req);
  const { type, quantity, reason } = req.body;
  const valid = ["in", "out", "adjust", "loss"];
  if (!valid.includes(type) || !quantity) return res.status(400).json({ error: "Dados inválidos." });

  const item = db.prepare("SELECT * FROM stock WHERE id = ? AND restaurant_id = ?").get(req.params.id, rid);
  ...
  db.prepare("UPDATE stock SET quantity = ? WHERE id = ?").run(Math.max(0, newQty), item.id);
  db.prepare(`
  ...
  const updated = db.prepare("SELECT * FROM stock WHERE id = ?").get(item.id);
...
router.get("/:id/movements", requirePermission("stock"), (req, res) => {
  const rid = restaurantScope(req);
  const item = db.prepare("SELECT id FROM stock WHERE id = ? AND restaurant_id = ?").get(req.params.id, rid);
  if (!item) return res.status(404).json({ error: "Item não encontrado." });

...
router.get("/", requirePermission("customers"), (req, res) => {
...
router.get("/ranking", requirePermission("customers"), (req, res) => {
  const rid = restaurantScope(req);
...
router.get("/:id", requirePermission("customers"), (req, res) => {
  const rid = restaurantScope(req);
  const customer = db.prepare("SELECT * FROM customers WHERE id = ? AND restaurant_id = ?").get(req.params.id, rid);
